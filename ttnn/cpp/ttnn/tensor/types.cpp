...
constexpr tt::DataFormat datatype_to_dataformat_converter(tt::tt_metal::DataType datatype) {
    switch (datatype) {
...
constexpr bool MemoryConfig::is_sharded() const {
...
constexpr bool MemoryConfig::is_l1() const { return buffer_type_ == BufferType::L1 or buffer_type_ == BufferType::L1_SMALL; }

constexpr bool MemoryConfig::is_dram() const { return buffer_type_ == BufferType::DRAM; }
...
constexpr bool operator!=(const MemoryConfig& config_a, const MemoryConfig& config_b) { return not(config_a == config_b); }
